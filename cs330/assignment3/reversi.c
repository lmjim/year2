...
int * getTurnInput(Board board, int player){
	...
	long x = 0;
        long y = 0;
        while(!valid){
                printf("Player %d\'s turn, choose row and col for new disc. Input 2 numbers: ", player); // Ask for input
                int max = 15;
                char input[max];
                fgets(input, max, stdin); // Read input
		char *token = NULL;
...
void runGame(Board board){
	...
	while(turnsRemaining){
		if(turnsRemaining == 1 || turnsRemaining == 3){
			...
			printf("No moves for player 1. Switch to player 2.\n");
		}
		if(turnsRemaining == 0){
			break;
		}
		if(turnsRemaining == 2 || turnsRemaining == 3){
			...
			printf("No moves for player 2. Switch to player 1.\n");
		}
	}
	// Game Over message
	gameOver(&score);
