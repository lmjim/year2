...
void sig_alrm(int signal){
	if(signal == SIGALRM){
		...
		// set next itimer values:
		timer.it_value.tv_sec = 0;
		timer.it_interval.tv_sec = 0;
		timer.it_interval.tv_usec = 0;
		int t = 0;
		if(prgms[CurrentProgram]->SystemTime != 0){
			t = prgms[CurrentProgram]->UserTime / prgms[CurrentProgram]->SystemTime;
		}
		if(t > 10 || t == 0){
			// if program is (most likely) cpubound, give it longer to run
			timer.it_value.tv_usec = 100000; // 100 milliseconds
			printf("\tTimer set for: 100ms\n");
		}
		else{
			// if program is (most likely) iobound, give it less time to run
			timer.it_value.tv_usec = 50000; // 50 milliseconds
			printf("\tTimer set for: 50ms\n");
		}
		...
		setitimer(ITIMER_REAL, &timer, 0); // start next timer
	}
}
...
int main(int argc, char **argv){
	...
	//struct itimerval timer; make global variable
        timer.it_value.tv_sec = 0;
        timer.it_value.tv_usec = 100000; // 100 milliseconds, default for first program
        timer.it_interval.tv_sec = 0;
        timer.it_interval.tv_usec = 0; // don't repeat, set new one each time
