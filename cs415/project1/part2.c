...
void sig(int signal){
	if(signal == SIGUSR1){
		StartRun = 1; // custom signal to set global variable for child processes
	}
}
...
void signalPrograms(PCB **prgms, int prgmCount, int signal){
	// send appropriate signal to all child processes
	int i = 0;
	for(; i<prgmCount; i++){
		kill(prgms[i]->PID, signal);
	}
	return;
}
...
int main(int argc, char **argv){
	...
	signal(SIGUSR1, sig); // subscribe to SIGUSR1 signal
	...
	printf("about to start programs\n");
	sleep(1);
	signalPrograms(prgms, prgmCount,  SIGUSR1); // signal children to start
	sleep(2);
	signalPrograms(prgms, prgmCount, SIGSTOP); // signal children to suspend
	printf("programs paused\n");
	sleep(2);
	printf("programs resuming\n");
	signalPrograms(prgms, prgmCount, SIGCONT); // signal children to continue
	waitForPrograms(prgmCount); // wait for all children to finish
	printf("programs done\n");
